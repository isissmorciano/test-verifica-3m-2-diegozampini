...
def valida_voto(voto_str) -> list[bool, str | float]:
    voto = voto_str.strip()
    for voto in range(0, 11):
        if 1 <= voto <= 10:
            return True
        else:
            return [False, "Voto non valido. Deve essere un numero intero tra 0 e 10."]

def formatta_lista(studenti) -> str:
    for studente in studenti:
        print(f"\nL'alunno {studente['nome']} ha preso {studente['voto']}")
...
#      print("Voto valido")
#  else:
#      print("Errore: il voto inserito non è valido.")
    main()
